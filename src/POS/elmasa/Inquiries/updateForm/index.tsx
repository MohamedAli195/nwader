...
import { IComplaint } from "../../../../app/features/complaints/complaintsSlice";
...
export default function UpdateInquiriesForm({ tempCat, onCloseUp }: IProps) {
  const [updateInquirie, { isLoading }] = useUpdateInquirieMutation();

  const { register, handleSubmit, setValue, control } = useForm<IComplaint>();

  const statusOptions: Option[] = [
    { value: "pending", label: "قيد الانتظار" },
    // { value: "progress", label: "جاري المعالجة" },
    { value: "answered", label: "تم الحل" },
    ...
    >
      ...
      <div>
        <Button className="w-full text-3xl" disabled={isLoading}>
          {isLoading ? "جاري التحديث..." : "تحديث حالة الاستفسار"}
        </Button>
      </div>
    </form>
  );
